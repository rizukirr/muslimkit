...
static int utf8_read_char(const char *s, uchar_t *out) {
  ...
  if (c[0] <= 0x7F) {
    ...
    *out = ((uchar_t)c[0] & 0x1F) << 6 | ((uchar_t)c[1] & 0x3F);
    ...
    /* E0..EF */
    *out = ((uchar_t)c[0] & 0xF) << 12 | ((uchar_t)c[1] & 0x3F) << 6 | ((uchar_t)c[2] & 0x3F);
    return 3;
  } else {
...
static void emit_array_indented(SB *out, const JsonNode *array, const char *space,
                                int indent_level);
static void emit_object(SB *out, const JsonNode *object);
static void emit_object_indented(SB *out, const JsonNode *object, const char *space,
                                 int indent_level);
...
static bool number_is_valid(const char *num) { return (parse_number(&num, NULL) && *num == '\0'); }
...
char *json_get_string(JsonNode *obj, const char *key) {
  JsonNode *node = json_find_member(obj, key);
...
bool json_get_bool(JsonNode *obj, const char *key) {
  JsonNode *node = json_find_member(obj, key);
